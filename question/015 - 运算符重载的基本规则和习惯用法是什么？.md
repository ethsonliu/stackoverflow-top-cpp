...
C++ 允许我们重载`operator new`和`operator delete`，以实现我们自己的目的。但是我不推荐去重载它们，除非你有一些性能问题和内存限制。在一些高性能算法中，它们往往会对其重载以获得对内存的高利用。
...
注意，在上面的示例代码中，operator delete 不会被调用，除非 X 的构造函数发生异常。
